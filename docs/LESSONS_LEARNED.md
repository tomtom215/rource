...
| 283 | 2026-02-11 | nJdJk | Interactive elements (tabs, chips, nav items) using `--text-secondary` (#9ca3ab) are hard to read at ~4.5:1 contrast | `--text-secondary` designed for descriptive text, not interactive elements that need to stand out | Introduce `--text-interactive` semantic design token: #c9d1d9 dark (~10.3:1), #32383f light (~8.5:1); apply to all tabs, chips, nav items |
| 284 | 2026-02-11 | nJdJk | Repo chips nearly invisible: dark bg (#21262d) + muted text (#9ca3ab) + small font (12px) + subtle border | Multiple factors compound: color, size, weight, border all contributing to poor visibility | Fix ALL contributing factors together: brighter text token, larger font-size, font-weight: 500, lighter border-color |
| 285 | 2026-02-11 | nJdJk | Inline `style=""` attributes in JS template literals (help-template.js) bypass CSS maintainability | 8+ inline styles embedded in template string; cannot be overridden by theme or media queries without `!important` | Extract ALL inline styles to CSS classes in component stylesheet; use semantic class names (.help-section-body, .help-shortcuts-grid, etc.) |
| 286 | 2026-02-11 | nJdJk | Chrome headless crashes with "Creating shared memory in /tmp/... Permission denied (13)" in sandboxed environments | Chrome needs shared memory backed by `/tmp`; sandboxed environments block `/tmp` writes | Use `headless_shell` variant + set `TMPDIR` to a writable directory (e.g., `tests/visual/.chrome-tmp`) |
| 287 | 2026-02-11 | nJdJk | VFL 4-viewport matrix (16 screenshots) misses viewport-specific bugs between breakpoints | 375px and 768px don't catch issues at 390px (iPhone 14), 1024px (iPad landscape), 1440px (MacBook Pro), 2560px (QHD) | Expand to 8 viewports × 2 views × 2 themes = 32 screenshots; created `capture-vfl-full.sh` |
| 288 | 2026-02-11 | nJdJk | Zero-copy stats buffer (Phase 84): 618.6x speedup for WASM↔JS frame stats — `[f32; 32]` + `Float32Array` view eliminates JSON serialization | `format!()` + string copy + `JSON.parse()` costs ~778ns/op on hot path; direct memory read costs ~1.26ns/op | Use fixed-size `[f32; N]` buffer in WASM struct; expose pointer via `getStatsBufferPtr()`; JS creates `Float32Array` view over `WebAssembly.Memory` |
| 289 | 2026-02-11 | nJdJk | `Float32Array` view over WASM memory gets detached when `memory.grow()` is called — reads return 0 or throw | WASM `memory.grow()` replaces the underlying `ArrayBuffer`; existing typed array views become detached | Add `ensureView()` check: if `statsView.buffer.byteLength === 0`, recreate the `Float32Array` view with the new memory buffer |
| 290 | 2026-02-11 | nJdJk | Bottom-sheet input field had no `:focus-visible` outline — keyboard users couldn't see focus state | `.bottom-sheet-input:focus` had custom border/box-shadow but no `:focus-visible` for keyboard navigation (WCAG 2.4.7) | Add `.bottom-sheet-input:focus-visible { outline: 2px solid var(--accent-blue); outline-offset: 2px; }` |
| 291 | 2026-02-11 | nJdJk | Light theme insights tabs not styled correctly — color falls through to dark theme value | `.insights-tab-btn` color set globally but no `.light-theme` override; light theme `--text-interactive` token alone insufficient for active state | Add explicit `.light-theme .insights-tab-btn` and `.light-theme .insights-tab-btn.active` overrides in themes.css |

---

*Last updated: 2026-02-11 | 291 entries | 15 categories*
