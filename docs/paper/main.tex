...
IEEE~754~\cite{ieee754_2019} binary32 (\texttt{f32}) represents numbers as a
1-bit sign, 8-bit exponent, and 23-bit significand. The gap between
mathematical reals and \texttt{f32} is the central challenge our
...
\end{tikzpicture}
\Description{Triangle diagram with Verus (498 proof fns) at top, Coq (2198 theorems) at bottom-left, and Kani (272 harnesses) at bottom-right, with bidirectional arrows showing complementary coverage.}
...
\end{tikzpicture}
\Description{Directed acyclic graph showing Coq compilation layers: Spec (9 files) at top, with arrows to R-Proofs (10 files, 1366 theorems), Z-Compute (9 files, 471 theorems), and FP Bounds (9 files, 361 theorems), and Z-Compute flowing to Extraction (9 files).}
...
\end{tikzpicture}
\Description{Linear pipeline diagram: Coq Z-specs (471 theorems) flows via Extraction to OCaml source, which flows via wasm\_of\_ocaml to WASM (6.8 KB).}
...
Fiat-Crypto). Our MetaCoq verified extraction test (9 ZVec2 operations)
relies on 5 axioms inherent to the MetaCoq erasure framework (e.g.,
\texttt{fake\_normalization}, \texttt{assume\_welltyped\_template\_program\_expansion}),
which are documented and expected per Sozeau et al.~\cite{sozeau2024metacoq}.
The extracted WASM library is a proof-of-concept demonstrating the end-to-end
pipeline; comparative runtime benchmarking against the production Rust-compiled
WASM module is left as future work.
...
via \texttt{grep} patterns (\texttt{\^{}(Theorem|Lemma|Local Lemma)} for
\coq{}, \texttt{proof fn} for \verus{}, \texttt{\#[kani::proof]} for \kani{})
from source files, ensuring reproducibility. The $\mathbb{R}$-based count
(1,366) includes 59 theorems from \texttt{Utils.v} (which has no separate
\texttt{\_Proofs.v} file); 11 additional specification-level lemmas in other
spec files are not counted. Not all theorems are equally deep ($\sim$15\%
are trivial, $\sim$55\% standard, $\sim$25\% non-trivial, $\sim$5\% deep).
The artifact's \texttt{reproduce-all.sh} independently counts and reports
theorem totals for cross-checking.
...
The hax tool~\cite{bhargavan2024hax} shares our multi-backend philosophy,
targeting F*, Coq, and ProVerif for cryptographic protocol verification, but
