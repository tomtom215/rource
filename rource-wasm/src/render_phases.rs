...
mod tests {
    ...
    fn test_lod_constants_are_reasonable() {
        ...
        assert!(LOD_MIN_FILE_RADIUS > 0.0, "File radius threshold must be positive");
        assert!(LOD_MIN_FILE_RADIUS < 1.0, "File radius threshold should be sub-pixel");

        // Directory radius threshold should be very small
        assert!(LOD_MIN_DIR_RADIUS > 0.0, "Dir radius threshold must be positive");
        assert!(LOD_MIN_DIR_RADIUS < 1.0, "Dir radius threshold should be sub-pixel");

        // Label thresholds should be larger than entity thresholds
        assert!(LOD_MIN_FILE_LABEL_RADIUS > LOD_MIN_FILE_RADIUS,
            "File label threshold should be larger than file threshold");
        assert!(LOD_MIN_DIR_LABEL_RADIUS > LOD_MIN_DIR_RADIUS,
            "Dir label threshold should be larger than dir threshold");
        assert!(LOD_MIN_USER_LABEL_RADIUS > LOD_MIN_USER_RADIUS,
            "User label threshold should be larger than user threshold");

        // Label thresholds should be readable sizes (at least a few pixels)
        assert!(LOD_MIN_FILE_LABEL_RADIUS >= 2.0, "File labels should be at least 2px");
        assert!(LOD_MIN_DIR_LABEL_RADIUS >= 2.0, "Dir labels should be at least 2px");
        assert!(LOD_MIN_USER_LABEL_RADIUS >= 2.0, "User labels should be at least 2px");

        // Zoom thresholds should be positive and less than 1.0
        assert!(LOD_MIN_ZOOM_FOR_FILE_BRANCHES > 0.0, "Branch zoom threshold must be positive");
        assert!(LOD_MIN_ZOOM_FOR_FILE_BRANCHES < 1.0, "Branch zoom threshold should be less than 1.0");
        assert!(LOD_MIN_ZOOM_FOR_DIR_BRANCHES > 0.0, "Dir branch zoom threshold must be positive");
        assert!(LOD_MIN_ZOOM_FOR_DIR_BRANCHES < 1.0, "Dir branch zoom threshold should be less than 1.0");
    }
    ...
    fn test_lod_file_visibility_at_various_zoom_levels() {
        ...
        assert!(screen_radius_normal >= LOD_MIN_FILE_RADIUS, "File visible at normal zoom");

        // At zoom = 0.01, file should be invisible (0.05px < 0.5px threshold)
        let zoom_far = 0.01;
        let screen_radius_far = file_world_radius * zoom_far;
        assert!(screen_radius_far < LOD_MIN_FILE_RADIUS, "File culled at far zoom");
        ...
        assert!((screen_radius_threshold - LOD_MIN_FILE_RADIUS).abs() < 0.001,
    ...
    fn test_lod_label_visibility_separate_from_entity() {
        ...
        if zoom_file_visible_label_hidden > LOD_MIN_FILE_RADIUS / file_world_radius {
            assert!(screen_radius < LOD_MIN_FILE_LABEL_RADIUS, "Label should be hidden");
            assert!(screen_radius >= LOD_MIN_FILE_RADIUS, "File should still be visible");
        }
    }
    ...
    fn test_lod_branch_rendering_zoom_thresholds() {
        ...
        assert!(1.0 >= LOD_MIN_ZOOM_FOR_FILE_BRANCHES, "File branches at normal zoom");

        // File branches should not render at very low zoom
        assert!(0.01 < LOD_MIN_ZOOM_FOR_FILE_BRANCHES, "File branches culled at very low zoom");

        // Directory branches have lower threshold (more important structure)
        assert!(LOD_MIN_ZOOM_FOR_DIR_BRANCHES <= LOD_MIN_ZOOM_FOR_FILE_BRANCHES,
