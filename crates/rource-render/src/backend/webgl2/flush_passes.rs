...
impl WebGl2Renderer {
    ...
    /// Flushes curve (spline) draw calls.
    ///
    /// This method renders all queued curve segments in a single instanced draw call,
    /// using GPU-tessellated Catmull-Rom splines.
    pub(super) fn flush_curves(&mut self) {
        if self.curve_instances.instance_count() == 0 {
            return;
        }

        let Some(program) = &self.curve_program else {
        ...
        // Setup VAO if needed
        if self.vao_manager.curve_vao.is_none() {
        ...
        let vao_changed = self
            .state_cache
            .bind_vao(gl, self.vao_manager.curve_vao.as_ref());
        if vao_changed {
            self.frame_stats.vao_switches += 1;
        } else {
            self.frame_stats.skipped_vao_binds += 1;
        ...
        // Draw instanced using triangle strip
        let instance_count = self.curve_instances.instance_count();
        gl.draw_arrays_instanced(
            WebGl2RenderingContext::TRIANGLE_STRIP,
            0,
            super::buffers::CURVE_STRIP_VERTEX_COUNT as i32,
            instance_count as i32,
        );

        // Track statistics
        self.frame_stats.draw_calls += 1;
        self.frame_stats.curve_instances += instance_count as u32;
        self.frame_stats.total_instances += instance_count as u32;
        self.frame_stats
            .active_primitives
